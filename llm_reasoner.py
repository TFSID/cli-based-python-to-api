...
Hasilkan HANYA signature dalam format:
async def endpoint_name(param1: Type = Form(...), param2: Type = Form(...), file: UploadFile = File(...)) -> dict:

Jangan sertakan penjelasan tambahan, hanya signature function.
"""
...
Hasilkan HANYA kode fungsi lengkap yang telah direfaktor, tanpa penjelasan tambahan.
Format:
...
def call_llm_for_signature(
    main_logic_chunks: List[str],
    llm_client: Optional[LLMClient] = None,
    use_mock: bool = False
) -> str:
    prompt = get_endpoint_signature_prompt(main_logic_chunks)
    
    if use_mock or llm_client is None:
        return mock_llm_response(prompt)
    
    return llm_client.generate(prompt, temperature=0.2, max_tokens=2048)


def call_llm_for_refactor(
    endpoint_signature: str,
    function_name: str,
    business_logic_chunk: str,
    llm_client: Optional[LLMClient] = None,
    use_mock: bool = False
) -> str:
    prompt = get_logic_refactor_prompt(endpoint_signature, function_name, business_logic_chunk)
    
    if use_mock or llm_client is None:
        return mock_llm_response(prompt)
    
    return llm_client.generate(prompt, temperature=0.2, max_tokens=4096)
