...
# Asumsikan logger sudah dikonfigurasi di main.py dan dapat diakses di sini.
logger = logging.getLogger('Pipeline.DataAnalyzer')
...
# --- Modul Khusus untuk Visualisasi VEM ---
...
# --- Fungsi Wrapper & Fungsi Adaptif ---

def create_vulnerability_charts(df_input: pd.DataFrame, output_excel_path: str) -> int:
    """Membuat dasbor VEM spesifik menggunakan kelas VEMDataVisualizer."""
    logger.info("Memulai visualisasi Dasbor VEM (Phase 4)...")
    if df_input.empty:
        logger.error("DataFrame input kosong untuk visualisasi VEM.")
        return 0
    vem_visualizer = VEMDataVisualizer(df_input)
    return vem_visualizer.create_vem_dashboard(output_excel_path)

def create_quick_dashboard(
    df: pd.DataFrame, output_path: str, column_configs: List[Dict[str, Any]] = None, auto_analyze: bool = True
) -> bool:
    """Membuat dasbor cepat dari DataFrame menggunakan konfigurasi atau analisis otomatis."""
    visualizer = DataVisualizer()
    if auto_analyze:
        for config in visualizer.auto_analyze_dataframe(df):
            visualizer.add_chart(config)
    # ... (logika untuk column_configs bisa ditambahkan di sini jika perlu) ...
    return visualizer.create_dashboard(output_path, dashboard_name='Quick_Dashboard')

def create_gemini_charts(
    df_input: pd.DataFrame, output_excel_path: str, gemini_analysis_result: Dict[str, Any], min_charts: int = 10
) -> int:
    """Membuat dasbor dinamis dengan menggunakan spesifikasi bagan dari Gemini."""
    ...
    if not viz_specs or not isinstance(viz_specs, list):
        ...
        for spec in viz_specs:
            if len(visualizer.charts) >= min_charts and min_charts > 0: break
            try:
                prep_func = spec.get('prep_function', 'none').lower()
                chart_type = ChartType[spec.get('chart_type', 'COLUMN').upper()]
                ...
                if data_to_viz is None or data_to_viz.empty: continue
                ...
                visualizer.add_chart(ChartConfig(
                    ...
                    sort_data=(chart_type in [ChartType.BAR, ChartType.COLUMN]),
                    sort_ascending=(chart_type == ChartType.BAR)
                ))
            except Exception as e:
                logger.error(f"Error tak terduga saat memproses spesifikasi bagan Gemini: {e}", exc_info=True)
    ...
    return len(visualizer.charts) if success else 0
